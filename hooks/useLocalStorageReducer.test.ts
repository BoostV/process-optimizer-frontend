...
describe('useLocalStorageReducer', () => {
  it('can be called once', () => {
    ...
    const { result } = renderHook(() => useLocalStorageReducer(reducer, initialState))
  ...
  it('can be called twice', () => {
    ...
    const reducer = (state: State, action: Action) => state
    const { result: result1 } = renderHook(() => useLocalStorageReducer(reducer, initialState))
    const { result: result2 } = renderHook(() => useLocalStorageReducer(reducer, {...initialState, experiment: {...initialState.experiment, id: '321' }}, '321'))
